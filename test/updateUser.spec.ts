...
describe('updateUser', () => {
	...
	describe('changeUserPassword', () => {
		...
		it('handles exception thrown within getAuthToken', (done) => {
			...
			changeUserPassword(params).catch((err) => {
				expect(err.message).to.equal(
					'getAuthToken - Auth service - Bad response'
				);
				done();
			});
		});
		...
		it('throws if invalid options supplied', (done) => {
			changeUserPassword({ apiKey: '' } as UpdateUserOptions).catch((err) => {
				expect(err.message).to.equal(
					'Invalid option(s): session, apiHost, apiClientId, userId, passwordData'
				);
				done();
			});
		});
	...
	describe('updateUserProfile', () => {
		...
		it('throws if invalid options supplied', (done) => {
			updateUserProfile({ userUpdate: '' } as UpdateUserOptions).catch(
				(err) => {
					expect(err.message).to.equal(
						'Invalid option(s): session, apiHost, apiKey, apiClientId, userId, userUpdate'
					);
					done();
				}
			);
